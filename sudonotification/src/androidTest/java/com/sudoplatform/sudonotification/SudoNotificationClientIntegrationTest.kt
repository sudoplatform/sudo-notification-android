...
class SudoNotificationClientIntegrationTest : BaseIntegrationTest() {
    ...
    @Test
    fun registerDeviceShouldNotThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun registerDevicesAgainShouldThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun deRegisterDevicesShouldNotThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun deRegisterInvalidDevicesShouldThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun updateDevicesShouldNotThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun updateDeviceWithoutRegisterShouldThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun updateInvalidDeviceShouldThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun getNotificationConfigListAfterRegisterShouldReturnEmpty(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun getNotificationConfigListOnInvalidDeviceShouldThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun setNotificationConfigShouldReturnSuccess(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
            ...
            val input =
                NotificationSettingsInput(deviceInfoProvider.bundleIdentifier, deviceInfoProvider.deviceIdentifier, confList, schema)
    ...
    @Test
    fun setNotificationConfigOnInvalidDeviceShouldThrow(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun setAndGetNotificationConfigShouldMatch(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
            ...
            val schema =
                listOf<NotificationMetaData>(TestNotificationMetaData(TEST_SERVICE_NAME), TestNotificationMetaData(TEST_ALT_SERVICE_NAME))
            val input =
                NotificationSettingsInput(deviceInfoProvider.bundleIdentifier, deviceInfoProvider.deviceIdentifier, config.configs, schema)
            notificationClient.setNotificationConfiguration(input)
    ...
    @Test
    fun getUserNotificationConfigListBeforeBeingSetShouldReturnNull(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun setAndGetUserNotificationConfigShouldMatch(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun getUserAndDeviceNotificationConfigBeforeBeingSetShouldReturnEmptyConfig(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
    ...
    @Test
    fun setAndGetUserAndDeviceNotificationConfigShouldMatch(): Unit =
        runBlocking {
            // Can only run if client config files are present
            Assume.assumeTrue(clientConfigFilesPresent())
